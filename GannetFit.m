    ...
    for jj = 1:length(target)
        ...
        for ii = 1:numscans
            ...
            try % pass to next dataset if errors occur
                ...
                fprintf('\n');
                warning('********** An error occurred while fitting %s in dataset: ''%s''. Check data. Skipping to next dataset in batch **********', target{jj}, MRS_struct.metabfile{ii});
                errorReport{tryCatchInd} = sprintf(['Filename: ' MRS_struct.metabfile{ii} '\n\n' getReport(ME,'extended','hyperlinks','on')]);
